...
//  socketDemo
//
//  Created by Ashish Pisey on 27/03/15.
...
class ViewController: UIViewController, SRWebSocketDelegate {
    ...
    let server = "localhost/Socket.IO%20Handshake" // don't include http://
    ...
    func initHandshake() {
        ...
        var handshakeTask:NSURLSessionTask = session!.dataTaskWithURL(url, completionHandler: { (data:NSData!, response:NSURLResponse!, error:NSError!) in
            if !(error != nil) {
